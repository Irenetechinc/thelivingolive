...
- Prayer points and devotionals are generated by the **rule-based prayer engine** (see below), not OpenAI
- Saved to Supabase for history
...
- Each generated prayer point/devotional has a 1-5 star "Was this helpful?" rating that feeds the engine's self-learning loop

### Rule-based prayer/devotion engine (self-evolving, no LLM/GPU)
- `server/src/lib/prayerEngine.js` + `server/src/data/prayerVerses.js` — detects a prayer category (Adoration/Thanksgiving/Petition/Intercession/Warfare) from the user's free text via keyword scoring, then selects and phrases prayer points/devotionals from a curated bank of real KJV scripture (pulled live from `server/src/data/bible/`, never hardcoded separately). No OpenAI/LLM call, no GPU — pure CPU-bound JS, safe on the cheapest always-on Railway instance.
- Endpoints: `POST /api/prayer-engine/prayer`, `POST /api/prayer-engine/devotion`, `POST /api/prayer-engine/feedback`. The original OpenAI-based `/api/ai/prayer` and `/api/ai/devotion` routes are left in place but unused by the app now.
- Self-learning (`server/src/lib/scheduler.js`, runs in-process via `node-cron`, no separate worker needed):
  1. **Real-time** — every feedback rating nudges that verse's weight in `verse_category_weights` immediately.
  2. **Hourly** — re-syncs the in-memory weights cache from Supabase.
  3. **Daily (03:00 server time)** — scans the day's 4-5 star feedback for words not yet in any category's keyword table; a word that recurs 3+ times with the same category gets promoted into `learned_keywords`.
- Scoped deliberately below a full genetic-algorithm/web-crawling pipeline (SearXNG, Crawl4AI, headless browsers) — that would need dedicated worker infrastructure beyond a single Railway web dyno. This is a real, working feedback loop, just a simpler one.
- **After pulling this change, re-run the (idempotent) `server/supabase/schema.sql` in the Supabase SQL Editor** — it adds `verse_category_weights`, `generation_feedback`, and `learned_keywords`.
