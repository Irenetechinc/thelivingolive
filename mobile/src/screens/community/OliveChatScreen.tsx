      ...
      <View style={pf.avatarRow}>
        ...
        {!editing
          ...
          : <View style={pf.editActions}>
              <Pressable style={pf.cancelEditBtn} onPress={() => { restoreDraftFromProfile(); setEditing(false); setShowOnboarding(false); }}>
...
export default function OliveChatScreen() {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          if (active) {
            setPinLocked(pinActive);
            setPinChecked(true);
            // A protected entry always starts with the splash, then presents
            // the PIN gate. This also covers navigating away and back while
            // the screen remains mounted.
            if (pinActive) setShowSplash(true);
          }
          ...
          try {
            const cached = await AsyncStorage.getItem(PIN_CACHE_KEY);
              // A missing cache means this device has never successfully
              // saved a PIN locally. Do not lock a first-time/offline user
              // out; once a PIN is set, the write path always creates this
              // cache entry and offline resume remains protected.
              const pinActive = cached === null ? false : JSON.parse(cached) === true;
            if (active) { setPinLocked(pinActive); setPinChecked(true); }
          } catch {
              if (active) { setPinLocked(true); setPinChecked(true); }
          }
        }
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener('change', async (nextState) => {
      const wasBackgrounded = previousAppState.current !== 'active' && nextState === 'active';
      ...
      try {
        ...
        const activePin = cached === null ? false : JSON.parse(cached) === true;
        setPinLocked(activePin);
        setPinChecked(true);
        if (activePin) setShowSplash(true);
      }
  ...
  if (pinChecked && !showSplash && pinLocked) {
    return <PinGate onVerified={() => { setPinLocked(false); setShowSplash(false); }} />;
  }
