...
// ── Message bubble ─────────────────────────────────────────────────────────────
function Bubble({ msg, isMine, showAvatar }: { msg: ChatMessage; isMine: boolean; showAvatar: boolean }) {
...
const bb = StyleSheet.create({
  ...
  time: { fontSize: 10, color: colors.inkFaint, marginTop: 4, textAlign: 'right' },
...
function RecordButton({ onRecordingDone }: { onRecordingDone: (uri: string, duration: number) => void }) {
  ...
  return recording ? (
    <View style={rb.recRow}>
      ...
      <Pressable style={rb.stopBtn} onPress={stop}><Text style={rb.stopIcon}>⏹</Text></Pressable>
    </View>
  ) : (
    <Pressable style={rb.micBtn} onLongPress={start} onPress={start}>
      <Text style={rb.micIcon}>🎙</Text>
    </Pressable>
  );
}
const rb = StyleSheet.create({
  ...
  stopIcon: { fontSize: 14, color: '#fff' },
});
...
export default function ChatRoomScreen({ route, navigation }: Props) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#F2EDE0' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
      <FlatList
        ref={flatRef}
        data={messages}
        keyExtractor={m => m.id}
        ...
        renderItem={({ item: msg, index }) => {
          ...
          return <Bubble msg={msg} isMine={isMine} showAvatar={showAvatar} />;
        }}
      ...
      <View style={[inp.bar, { paddingBottom: insets.bottom > 0 ? insets.bottom : spacing.sm }]}>
        ...
        <View style={inp.row}>
          <Pressable style={inp.attachBtn} onPress={sendImage}><Text style={inp.attachIcon}>📎</Text></Pressable>
          <TextInput
            style={inp.input}
            value={text}
          ...
          {text.trim() ? (
            <Pressable style={[inp.sendBtn, sending && { opacity: 0.6 }]} onPress={send} disabled={sending}>
              {sending ? <ActivityIndicator size="small" color="#fff" /> : <Text style={inp.sendIcon}>↑</Text>}
            </Pressable>
          ) : (
