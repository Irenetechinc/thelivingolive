...
// ── Line Icon renderer for category icons (line:ID via MaterialCommunityIcons) ─
const LINE_ICON_MAP: Record<string, string> = {
  book: 'book-open-page-variant',
  shirt: 'tshirt-crew-outline',
  music: 'music-note-half-dotted',
  cup: 'coffee-outline',
  gift: 'gift-outline',
  home: 'home-outline',
  cart: 'cart-outline',
  ...
  scroll: 'scroll-outline',
  lamp: 'lamp-outline',
  seed: 'sprout',
  anchor: 'anchor',
  globe: 'earth',
  'book-heart': 'book-heart-outline',
  video: 'video-outline',
  pen: 'pencil-outline',
  'gift-box': 'package-variant-closed',
  ear: 'ear-hearing',
  ...
  medal: 'medal-outline',
  houseware: 'pot-steam-outline',
  lemon: 'fruit-watermelon',
  anoint: 'oil-lamp',
  tower: 'castle',
  ship: 'boat',
...
function CategoryIcon({ icon, size = 14, color }: { icon: string; size?: number; color?: string }) {
  if (!icon) return <MaterialCommunityIcons name="shopping-outline" size={size} color={color ?? colors.inkSoft} />;
  if (typeof icon === 'string' && icon.startsWith('line:')) {
    const id = icon.slice(5);
    const name = (LINE_ICON_MAP[id] ?? LINE_ICON_MAP.default) as keyof typeof MaterialCommunityIcons.glyphMap;
    return <MaterialCommunityIcons name={name} size={size} color={color ?? colors.inkSoft} />;
  }
...
function ProductCard({ product, onPress, delay = 0 }: {
  ...
  async function handleAddToCart(e: any) {
    ...
    try {
      await addShopCartItem({ productId: product.id, quantity: 1 });
      Alert.alert('Added to cart', `${product.title} was added to your cart.`);
    } catch (err: any) {
      Alert.alert('Cart Error', err?.message ?? 'Could not add item');
      ...
      <Pressable style={pc.card} onPress={onPress} onPressIn={handlePressIn} onPressOut={handlePressOut}>
        ...
        <View style={pc.info}>
          ...
          {product.product_type === 'physical' && (
            <Pressable
              style={[pc.cartBtn, addingToCart && { opacity: 0.6 }]}
            ...
            >
              {addingToCart
                ...
                : <>
                    <Text style={pc.cartBtnIcon}>🛒</Text>
                    <Text style={pc.cartBtnText}>Add to Cart</Text>
                  </>}
            </Pressable>
          )}
        </View>
...
const pc = StyleSheet.create({
  card: { width: 158, height: 290, marginRight: spacing.sm, backgroundColor: '#fff', borderRadius: radii.lg, overflow: 'hidden', ...shadows.card },
  ...
  info: { padding: spacing.sm, height: 130, justifyContent: 'flex-start' },
  title: { fontSize: 13, fontWeight: '700', color: colors.ink, lineHeight: 18, marginBottom: 4, minHeight: 36 },
  church: { fontSize: 10, color: colors.inkFaint, marginBottom: spacing.sm },
  cartBtn: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  return (
    <Animated.View style={{
      ...
      <Pressable
        ...
        onPress={onPress}
      >
        <CategoryIcon icon={cat.icon} size={14} color={isActive ? '#fff' : colors.inkSoft} />
    ...
    <View style={catsec.wrap}>
      <View style={catsec.header}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 }}>
          <CategoryIcon icon={category.icon} size={18} color={S.gold} />
          <Text style={catsec.title}>{category.name}</Text>
        </View>
...
function MyOrdersSheet({ orders, loading, onClose, onDownload, onVerify }: {
  ...
  const trackingStepColor = (idx: number, curIdx: number) =>
    idx <= curIdx ? '#2E7D32' : colors.inkFaint;
  const trackingSteps = ['Order placed', 'Payment confirmed', 'Processing', 'Shipped', 'Out for delivery', 'Delivered'];

  function getTrackingIdx(o: ShopOrder) {
    if (o.status !== 'paid' && o.status !== 'collected') return 0;
    if (!o.tracking_status) return 1;
    const s = o.tracking_status.toLowerCase();
    if (s.includes('deliv') && !s.includes('out')) return 5;
    if (s.includes('out') || s.includes('final')) return 4;
    if (s.includes('ship')) return 3;
    if (s.includes('process') || s.includes('pack')) return 2;
    if (s.includes('payment') || s.includes('paid') || s.includes('confirm')) return 1;
    if (s.includes('order') || s.includes('received') || s.includes('ready')) return 1;
    return 1;
  }
          ...
          renderItem={({ item: o }) => {
            ...
            return (
              <View style={{ backgroundColor: '#fff', borderRadius: radii.lg, padding: spacing.md, marginBottom: spacing.md, ...shadows.subtle }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                      ...
                      {o.fulfillment_method && (
                        <Text style={{ fontSize: 11, color: colors.inkFaint }}>
                          · {o.fulfillment_method === 'pickup' ? '📍 Pickup' : '🚚 Delivery'}
                        </Text>
                      )}
                      {o.invoice_number && (
                        <Text style={{ fontSize: 10, color: colors.inkFaint, fontStyle: 'italic' }}>
                ...
                {isPickupPaid && (o.collection_code || o.collection_qr) && (
                  <View style={{ marginTop: spacing.sm, padding: spacing.md, backgroundColor: '#FFF8E1', borderRadius: radii.md, borderWidth: 1, borderColor: '#FFD54F' }}>
                    <Text style={{ fontSize: 12, fontWeight: '800', color: S.dark, marginBottom: 4 }}>⛪ COLLECTION DETAILS</Text>
                    {o.collection_code && (
                      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                        <Text style={{ fontSize: 12, color: colors.inkSoft }}>Collection Code:</Text>
                        <Text style={{ fontSize: 18, fontWeight: '900', color: S.dark, letterSpacing: 2 }}>{o.collection_code}</Text>
                      </View>
                    )}
                    {(o.collection_qr || o.collection_code) && (
                      <View style={{ alignItems: 'center', marginVertical: spacing.sm }}>
                        <Image
                          source={{ uri: qrImageUrl(o.collection_qr || o.collection_code!) }}
                          style={{ width: 160, height: 160, borderRadius: 8, backgroundColor: '#fff' }}
                          resizeMode="contain"
                        />
                        <Text style={{ fontSize: 10, color: colors.inkFaint, marginTop: 6, textAlign: 'center' }}>
                          Show this QR / barcode for staff to scan and confirm payment
                        </Text>
                      </View>
                    )}
                    <Text style={{ fontSize: 11, color: colors.inkSoft, lineHeight: 15, marginTop: 2 }}>
                      Present this code at the church office to collect your product. Staff can scan the QR or enter the code in Org Admin.
                    </Text>
                    {o.collection_qr && (
                      <Pressable
                        style={{ marginTop: spacing.sm, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 8, borderRadius: radii.md, backgroundColor: S.gold }}
                        onPress={() => Linking.openURL(o.collection_qr!)}
                      >
                        <Text style={{ fontSize: 12, color: '#fff', fontWeight: '700' }}>Open verify link</Text>
                      </Pressable>
                    )}
                  </View>
                )}
                {o.fulfillment_method === 'delivery' && (
                  <View style={{ marginTop: spacing.sm }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.xs }}>
                      <Text style={{ fontSize: 11, fontWeight: '700', color: colors.ink }}>Order Tracking</Text>
                      {o.paid_at && (
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', paddingHorizontal: 2 }}>
                          ...
                          <View key={step} style={{ flex: 1, alignItems: 'center', position: 'relative' }}>
                            {idx < trackingSteps.length - 1 && (
                              <View style={{
                                ...
                                backgroundColor: idx < trackingIdx ? '#2E7D32' : '#DDD',
                              }} />
                            )}
                            <View style={{
                              width: 16, height: 16, borderRadius: 8,
                              alignItems: 'center', justifyContent: 'center',
                              backgroundColor: isActive ? trackingStepColor(idx, trackingIdx) : colors.parchmentDark,
                              zIndex: 1,
                            }}>
                              <Text style={{ fontSize: 9, color: '#fff', fontWeight: '900' }}>
                                {trackingStepIcon(idx, trackingIdx)}
                              </Text>
                            </View>
                            <Text style={{
                              fontSize: 9, marginTop: 4, textAlign: 'center',
                              color: isActive ? colors.ink : colors.inkFaint, fontWeight: isCurrent ? '700' : '400',
                              maxWidth: '100%',
                            }} numberOfLines={2}>{step}</Text>
                          </View>
                        );
                      })}
                    </View>
                    {o.tracking_events && o.tracking_events.length > 0 && (
                      <View style={{ marginTop: spacing.sm, paddingTop: spacing.sm, borderTopWidth: 1, borderTopColor: colors.parchment }}>
                        {o.tracking_events.slice(-2).map((ev, i) => (
                          <View key={i} style={{ flexDirection: 'row', gap: 6, marginBottom: 2 }}>
                            <Text style={{ fontSize: 10, color: colors.inkFaint, fontWeight: '700' }}>
                              {new Date(ev.at).toLocaleDateString()}
                            </Text>
                            <Text style={{ fontSize: 10, color: colors.inkSoft, flex: 1 }} numberOfLines={1}>
  ...
  async function doPurchase(addr?: string, options = purchaseOptions) {
    ...
    if (selectedProduct.product_type === 'physical') {
      ...
      if (!shippingPhone.trim()) {
        Alert.alert('Phone required', 'Enter a phone number so the church can reach you.');
        return;
      }
      if (fulfillmentMethod === 'delivery' && !(addr || deliveryAddr).trim()) {
    ...
    try {
      ...
      if (result.free) {
        ...
        const pickupNote = result.collectionCode
          ? `\n\nCollection code: ${result.collectionCode}`
          : '';
        Alert.alert('🎁 Success', `"${selectedProduct.title}" has been added to your orders!${result.invoiceNumber ? `\nInvoice: ${result.invoiceNumber}` : ''}${pickupNote}\n\nAn invoice was sent to your email.`);
        return;
      }
      if (result.paymentLink) {
        ...
        Alert.alert(
          'Payment Opened',
          `Invoice ${result.invoiceNumber ?? ''} was sent to your email. Complete the payment in your browser, then return to My Orders to verify.`.replace(/\s+/g, ' ').trim(),
          [{ text: 'OK' }]
        );
      }
